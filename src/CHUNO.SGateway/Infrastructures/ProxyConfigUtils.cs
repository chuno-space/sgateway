...
namespace CHUNO.SGateway.Infrastructures
{
    ...
    {
        public static ConfigurationSnapshot ReadConfiguration(IConfiguration configuration)
        {
            var newSnapshot = new ConfigurationSnapshot();

            foreach (var section in configuration.GetSection("Clusters").GetChildren())
            {
                newSnapshot.Clusters.Add(CreateCluster(section));
            }

            foreach (var section in configuration.GetSection("Routes").GetChildren())
            {
